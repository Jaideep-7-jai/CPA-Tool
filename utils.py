...
def ensure_output_dir(output_dir: str,criteria_type) -> Path:
    """Create output directory if not exists + add timestamp for safety"""
    ...
    # If exists, create timestamped subdir for multiple runs
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    safe_dir = path / f"run_{criteria_type}_{timestamp}"
    safe_dir.mkdir(exist_ok=True)
    logging.info(f"📁 Using timestamped directory: {safe_dir}")
    return safe_dir




def run_command(cmd: Union[list, str],cwd: Optional[str] = None,timeout: int = 3600,stdout: Optional[int] = None) -> str:
...
def download_combine(s3_path: str, output_file: str, cwd: str) -> Tuple[str, int]:
    ...
    # Step 3: Combine files + COUNT LINES (KEEP CSV temporarily)
    start = time.time()
    ...
    # ✅ RETURN ZIP filename + line count
    return zip_name, line_count


def get_db_connection(channel: str) -> Dict:
...
def send_email(subject: str, body_text: str, is_error: bool = False):
    """Email notification"""
    try:
        msg = MIMEMultipart()
...
def send_success_email(subject: str, files: List[str], output_dir: str):
    """Success email with file details"""
    safe_files = [f for f in files if os.path.exists(f)]
    file_details = "\n".join([f"📄 {os.path.basename(f)} ({os.path.getsize(f)/1e6:.1f} MB)" for f in safe_files])
...
def send_error_email(subject: str, error_msg: str):
    """Error email"""
...
def load_zip_to_pg(zip_file: str, table_name: str, pg_config: Dict, truncate: bool = True):
